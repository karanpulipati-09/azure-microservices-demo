...
> **Note:** Steps 1–4 below extend `bootstrap/github-oidc-setup.md` with Azure CLI equivalents of the same portal steps. If you change the OIDC/IAM setup in one doc, update the other so they don't drift apart.
...
# App registration only creates an application object — role assignments need
# the corresponding service principal (enterprise application), so create it first
az ad sp create --id "$AZURE_CLIENT_ID"

# Get the service principal's object ID (not the application object ID)
...
The `Terraform Apply` workflow only triggers on pushes that touch `terraform/**`, so the change must be inside that directory:
...
# Make a minor commit inside terraform/ to trigger the workflow
echo "# Deployment started" >> terraform/locals.tf
